...
const ChartPreviewPanel = ({ sharedState, setSharedState }) => {
    ...
    const handleMouseDown = (e, id) => {
        if (e.target.classList.contains('react-resizable-handle')) return;
    ...
    const renderChartContent = (chartObj) => {
        ...
        if (chartObj.chartType === 'bar') {
            return (
                <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={data} margin={{ top: 20, right: 40, bottom: 50, left: 20 }}>
        ...
        if (chartObj.chartType === 'radar') {
            ...
            return (
                <ResponsiveContainer width="100%" height="100%">
                    <RadarChart data={data} margin={{ top: 30, bottom: 30, left: 30, right: 30 }}>
                        <PolarGrid />
            ...
            <Grid container spacing={2}>
                <Grid item xs={12} md={4}>
                    <Paper elevation={2} sx={{ p: 2 }}>
                        ...
                        <Button variant="contained" onClick={addMappedTextItem} fullWidth sx={{ mt: 1 }}>🧲 맵핑 텍스트 추가</Button>
                        <Button variant="outlined" onClick={() => setModalOpen(true)} fullWidth sx={{ mt: 1 }}>📊 그래프 추가</Button>
